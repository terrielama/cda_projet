...
const Cart = () => {
  ...
  const updateQuantity = async (itemId, delta) => {
    ...
    if (!cart_code) return;
    try {
      await api.post(`/update_quantity?cart_code=${cart_code}`, {
        item_id: itemId,
        delta,
      });
      fetchCart();
  ...
  const removeProduct = async (itemId) => {
    ...
    if (!cart_code) return;
    try {
      await api.post(`/remove_item?cart_code=${cart_code}`, { item_id: itemId });
      fetchCart();
    } catch (error) {
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      console.error(error);
      setOrderMessage("Erreur lors de la commande.");
    } finally {
      setLoading(false);
    }
  };
